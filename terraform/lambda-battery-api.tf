    ...
    Statement = [
      {
        ...
        Effect   = "Allow"
        Resource = "arn:aws:s3:::${aws_s3_bucket.device_shared.id}/*"
      },
    ]
  })
...
data "archive_file" "lambda_battery_api" {
  ...
  source_dir  = "${path.module}/lambda_battery_api"
  output_path = "${path.module}/templates/lambda/battery_api.zip"
}

resource "aws_s3_object" "lambda_battery_api" {
  bucket = aws_s3_bucket.device_shared.id

  key    = "lambda/battery_api.zip"
